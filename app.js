...
async function afterWrite(actionName){
  ...
  if(mode === "onAction" || mode === "both"){
    // v1.3.7: ไม่รอ backup ให้เสร็จก่อน เพื่อให้ปุ่ม Save / ส่งยอดตอบสนองเร็วขึ้นและไม่กระตุก
...
async function testBackupUrl(){
  ...
  $("#backupState").innerHTML = `<div class="state warn">เปิดหน้าทดสอบ Apps Script แล้ว หน้าใหม่ต้องขึ้น Love Matcha Sales Backup v1.3.7 และมี jsonFileName / folderUrl ถ้ายังขึ้น v1.2 หรือยังมี sheetName แปลว่ายัง Deploy โค้ด Apps Script ใหม่ไม่สำเร็จ</div>`;
}
...
async function handleRestoreFile(){
  ...
  if(!lower.endsWith(".json")){
    ...
    $("#restorePreview").innerHTML = `<div class="state error">v1.3.7 รองรับ Restore เฉพาะไฟล์ .json เท่านั้น</div>`;
